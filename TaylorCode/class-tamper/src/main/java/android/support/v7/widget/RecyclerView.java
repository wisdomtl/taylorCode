...
//                a.recycle();
    ...
    public RecyclerView.ItemDecoration getItemDecorationAt(int index) {
        return index >= 0 && index < this.mItemDecorations.size() ? (RecyclerView.ItemDecoration)this.mItemDecorations.get(index) : null;
    }
        ...
        if (next != null && next != this) {
            if (focused == null) {
                ...
                if (this.isPreferredNextFocusAbsolute(focused, next, absHorizontal)) {
                    ...
                    return direction == 2 ? this.isPreferredNextFocusAbsolute(focused, next, 130) : this.isPreferredNextFocusAbsolute(focused, next, 33);
                }
            }
        } else {
            return false;
    ...
    private boolean isPreferredNextFocusAbsolute(View focused, View next, int direction) {
        ...
        switch(direction) {
            ...
            default:
                throw new IllegalArgumentException("direction must be absolute. received:" + direction + this.exceptionLabel());
        }
    }
